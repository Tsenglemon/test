...
//    NSDate *now = [NSDate date];//当前日期
    ...
    NSString *curYearStr = [NSString stringWithFormat:@"%ld",curYear];
    NSString *curMonthStr = [NSString stringWithFormat:@"%ld",curMonth];
    //如果这周周日的日期超过本月天数
    if (sunday > dayNumofCurMonth)
    {
        ...
        if (curMonth + 1 > 12) {
            nextMonthStr = @"1";
            nextYearStr = [NSString stringWithFormat:@"%ld",curYear + 1];
        }else{
            nextMonthStr = [NSString stringWithFormat:@"%ld",curMonth + 1];
            nextYearStr = curYearStr;
        }
        int i;
        for (i = 0; i < 7; i++) {//本月
            if (monday + i <= dayNumofCurMonth) {
                [array addObject:@[curYearStr,curMonthStr,[NSString stringWithFormat:@"%ld",monday + i]]];
            }else{
                break;
            }
        }
    ...
    if (monday <= 0) {
        ...
        if (curMonth - 1 <= 0) {
            preMonthStr = @"12";
            preYearStr = [NSString stringWithFormat:@"%ld",curYear - 1];
        }else{
            preMonthStr = [NSString stringWithFormat:@"%ld",curMonth - 1];
            preYearStr = curYearStr;
        }
        int i;
        for (i = 0; i < 7; i++) {//上月
            if (curMonday + i <= dayNumOfPreMonth) {
                [array addObject:@[preYearStr,preMonthStr,[NSString stringWithFormat:@"%ld",monday + i]]];
            }else{
                break;
            }
        }
...
+ (NSArray *)getDatesSinceCurence:(int)weeks
{
    ...
    NSString *month = [NSString stringWithFormat:@"%ld月",[firstDayComp month]];
    ...
    for (int i = 0; i< 7; i++) {
        ...
        [array addObject:[NSString stringWithFormat:@"%ld",[everCom day]]];
    }
