...
- (void)layoutSubviews{
    ...
    _pickerview.frame = CGRectMake(0, 86, self.frame.size.width, 200);
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    ...
    _year = _year +delta;
    self.yearLab.text = [NSString stringWithFormat: @"%ld", _year];
    _month = row % self.monthArr.count + 1;
    self.monthLab.text = [NSString stringWithFormat: @"%ld月", _month];
    lastyear = newyear;
}
